function point = sphericalProduct_sampling_tapered(para, arclength)

[~,Xc1] = sphericalProduct_sampling(para, arclength);
[~,Xc2] = sphericalProduct_sampling([para(1:11) -para(12) -para(13)], arclength);

point = [Xc1(:,Xc1(3,:)>=0), Xc2(:,Xc2(3,:)<=0)];
R = eul2rotm(para(6 : 8));
t = para(9 : 11);
point = R * point + t';
...
function [point, Xc, normal, etaCorrPoint, omegaCorrPoint] = sphericalProduct_sampling(para, arclength)
...
% -------------------------------------------------------------------------
epsilon = max(para(1 : 2), [0.01, 0.01]);
a = para(3 : 5);
R = eul2rotm(para(6 : 8));
t = para(9 : 11);
...
for i = 1 : size(a_mod, 2)
    x_scale = point1(2,i) / a(3) * para(12) + 1;
    y_scale = point1(2,i) / a(3) * para(13) + 1;
    [~, theta2, critical2] = uniformSampledSuperellipse(epsilon(2), [a(1) * x_scale, a(2) * y_scale], [0 0 0], arclength / a_mod(i), 0);
    omega_flipped{i} = [theta2(1 : critical2 - 1), flip(theta2(critical2 : end))];
    
    points_fw2 = angle2points(theta2(1 : critical2 - 1), [a(1) * x_scale, a(2) * y_scale], epsilon(2));
    normals_fw2 = angle2normals(theta2(1 : critical2 - 1), [a(1) * x_scale, a(2) * y_scale], epsilon(2));
    
    points_bw2 = flip(angle2points(theta2(critical2 : end), [a(2)*y_scale, a(1)*x_scale], epsilon(2)), 2);
    normals_bw2 = flip(angle2normals(theta2(critical2 : end), [a(2)*y_scale, a(1)*x_scale], epsilon(2)), 2);
...
Xc = [xc;yc;zc];
point = R * Xc + t';
% xc = xc .* (para(12)/para(5) * zc + 1);
% yc = yc .* (para(13)/para(5) * zc + 1);
% point = [xc;yc;zc];
% point = R * point + t';
